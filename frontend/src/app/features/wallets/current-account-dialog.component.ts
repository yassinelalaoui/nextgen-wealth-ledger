...
@Component({
  selector: 'app-current-account-dialog',
  standalone: true,
  ...
  template: `
    <h2 mat-dialog-title>New Current Account</h2>
    <mat-dialog-content>
      <form [formGroup]="form" style="display:flex;flex-direction:column;gap:12px;min-width:320px;padding-top:8px">
        ...
        <mat-form-field appearance="outline">
          ...
          <mat-error *ngIf="form.get('initialBalance')?.hasError('min')">Must be ≥ 0</mat-error>
        </mat-form-field>
        <mat-form-field appearance="outline">
          <mat-label>Overdraft Limit</mat-label>
          <input matInput type="number" formControlName="overDraft" min="0">
          <mat-error *ngIf="form.get('overDraft')?.hasError('required')">Required</mat-error>
        </mat-form-field>
        <mat-form-field appearance="outline">
...
export class ActiveWalletDialogComponent implements OnInit {
  form!: FormGroup;
  clients: ClientDTO[] = [];

  constructor(
    private fb: FormBuilder,
    public dialogRef: MatDialogRef<ActiveWalletDialogComponent>,
  ...
  ngOnInit() {
    this.form = this.fb.group({
      ...
      overDraft: [1000, [Validators.required, Validators.min(0)]],
      currency: ['MAD']
    });
